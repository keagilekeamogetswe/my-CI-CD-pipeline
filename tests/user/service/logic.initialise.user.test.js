...
describe("User initialiseUserAccount", () => {
  ...
  it("Initialise user", async () => {
    ...
    const set_pass_token =
      await verification.confirmUserCreationVerificationCode(
        token,
        current_code,
        IPv4,
      );
    const user_password = "user private passwrod";
    const mysql_original_begin_trasaction_call =
    ...
    const [device_info, device_fingerprint] = [
      "testing mobile device",
      { passing_fp: {}, active_fp: {} },
    ];
    const refresh_token =
      await user_sessiont_instance.setPasswordToUserSessionStart(
        set_pass_token,
        user_password,
        { IPv4, device_info, device_fingerprint },
      );
    ...
    const { payload: user_session_payload } = await JWTHelper.decode(
      refresh_token,
    ...
    const [[session]] = await mysql_connection.execute(
      "SELECT * FROM user_session WHERE user_id = ? AND jti = ? ;",
      [user_id, user_session_payload.jti],
    );
    expect(session.jti).toBe(user_session_payload.jti);
    expect(session.session_type).toBe("primary");
    expect(Date(session.created_at)).toStrictEqual(
      Date(user_session_payload.iat),
    );
  });
});
